...
:::note
The DB-Schema is still a work in progress and will be updated as we add all the necessary features.
The most up-to-date schema can always be found [here](https://github.com/CTF-Citadel/webapp/blob/main/db/schema.sql)
:::

:::caution
Prisma is due for depreciation as soon as Lucia V3 is released, which includes an adapter for [Drizzle ORM](https://orm.drizzle.team/)

We will go this route as the development team has not been satisifed with the performance and usage of Prisma so far.
:::
...
:::note
As we adapt to newer versions of Astro which handles middleware differently, this could need adaption both on our side and Lucia upstream.
:::
