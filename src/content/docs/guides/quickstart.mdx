...
For tinkering with the Platform itself, one may want to spin up the other profile in the provided `docker-compose.yaml`, to spin up a standalone MariaDB instance.

```bash
docker-compose --profile dev up -d --build
```

This database will then listen on `localhost:3306`, and can be accessed by starting the WebApp in development mode from the project root.
